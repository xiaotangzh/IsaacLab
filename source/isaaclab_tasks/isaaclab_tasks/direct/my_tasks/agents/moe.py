...
class MOE(Agent):
    def __init__(
        ...
        """
        _cfg = copy.deepcopy(MOE_DEFAULT_CONFIG)
        ...
        # moe specific
        self.gating_size = self.cfg["gating_size"]
        self.num_experts= self.cfg["num_experts"]
        self.gatings_size = self.cfg["gatings_size"] 
        self.sub_action_size = self.cfg["sub_action_size"] 
        self.expert_names = list(self.experts.keys())
        self.moe_batch_size = self.cfg["moe_batch_size"]

        # set up automatic mixed precision
        self._device_type = torch.device(device).type
        if version.parse(torch.__version__) >= version.parse("2.4"):
            self.scaler = torch.amp.GradScaler(device=self._device_type, enabled=self._mixed_precision)
            self.scaler_moe = torch.amp.GradScaler(device=self._device_type, enabled=self._mixed_precision)
        else:
        ...
        if self.policy and self.value and self.experts:
            ...
            if self._learning_rate_scheduler:
                ...
                self.scheduler_moe = self._learning_rate_scheduler( #todo double check
                    self.optimizer_moe, **self.cfg["learning_rate_scheduler_kwargs"]
                )
            self.checkpoint_modules["optimizer"] = self.optimizer
            self.checkpoint_modules["optimizer_moe"] = self.optimizer_moe
