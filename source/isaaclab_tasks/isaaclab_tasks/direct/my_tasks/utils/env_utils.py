...
def compute_pairwise_joint_distance_env_weight(x, func: str="mean", sqrt=True, upper_bound=1.5): # x: [envs, keys * keys]
    '''
    Compute averaged pairwise joint distance weight for each env
    '''
    assert len(x.shape) <= 2, "x must be 2D tensor"
    value = compute_pairwise_joint_distance_weight(x, sqrt, upper_bound=upper_bound)
    match func:
        case "mean": return torch.mean(value, dim=1)
        case "max" : return torch.max(value, dim=1)[0]

def animate_pairwise_joint_distance_heatmap(x, key_names=None, sqrt=False, upper_bound=1.5): # x: [keys * keys]
...
def plot_pairwise_joint_distance_weight():
    ...
    y = compute_pairwise_joint_distance_weight(torch.from_numpy(x)).numpy()
